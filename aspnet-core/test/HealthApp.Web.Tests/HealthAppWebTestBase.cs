...
{
    public abstract class HealthAppWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static HealthAppWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(HealthAppWebMvcModule).Assembly.FullName);
        }
        ...
        {
            using (var context = IocManager.Resolve<HealthAppDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<HealthAppDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<HealthAppDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<HealthAppDbContext, Task<T>> func)
        {
            T result;
